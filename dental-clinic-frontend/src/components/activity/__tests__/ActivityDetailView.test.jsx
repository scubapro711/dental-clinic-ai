...
describe('ActivityDetailView Component', () => {
  ...
  describe('Interactive Elements', () => {
    ...
    test('timeline steps are clickable', () => {
      const firstStep = screen.getAllByText('Request received')[0];
      fireEvent.click(firstStep);
      // Should not throw error
      expect(firstStep).toBeInTheDocument();
    });

    test('explanation toggle works', async () => {
  ...
  describe('Error Handling', () => {
    test('handles missing activity ID gracefully', async () => {
      render(
        <ActivityDetailView 
          onClose={mockOnClose}
          onHumanHandoff={mockOnHumanHandoff}
  ...
  describe('Visual Layout', () => {
    ...
    test('responsive design elements are present', () => {
      ...
      const modal = screen.getByRole('dialog');
      expect(modal.firstChild).toHaveClass('max-w-6xl');
    });
  });
});
