 ...
 * 
 * Main dashboard view with widget management and customization
 */
import React, { useState } from 'react';
import { Plus } from 'lucide-react';
import { useAuth } from '../../../hooks/useAuth';
import { useAgentWebSocket } from '../../../hooks/agentic_dashboard/useAgentWebSocket';
...
export const DashboardView = ({ onPatientSelect }) => {
  ...
  const { user } = useAuth();
  const { activities } = useAgentWebSocket(true, user?.organization_id);

  const allWidgetsDef = [
    { 
      id: 'patients', 
      label: 'יומן מטופלים', 
      component: <TodaysPatientsWidget onSelect={onPatientSelect}/> 
    },
    { 
      id: 'decisions', 
      label: 'תור החלטות', 
    ...
    { 
      id: 'activity', 
      label: 'פיד סוכנים', 
      component: <AgentActivityWidget activities={activities}/> 
    },
    { 
      id: 'systems', 
      label: 'תשתיות', 
      component: <ClinicalSystemWidget/> 
    },
    { 
      id: 'tuning', 
      label: 'בקרת סוכנים', 
          ...
          <div className="flex gap-2 flex-wrap justify-end">
            {hiddenWidgets.map(w => (
              <button 
                ...
                className="flex items-center gap-1 bg-white border border-dashed border-slate-300 text-slate-500 text-xs px-3 py-1.5 rounded-lg hover:border-blue-400 hover:text-blue-600 transition dark:bg-slate-800 dark:border-slate-600 dark:text-slate-400"
              >
